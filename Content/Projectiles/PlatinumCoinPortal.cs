...
{
    public class PlatinumCoinPortal : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Platinum Coin Portal");
        ...
        public override void AI()
        {
			...
			else if (projectile.ai[0] <= 210f)
			{
				if (Main.netMode != 1 && (projectile.localAI[0] += 1f) >= projectile.ai[1])
				{
					...
					int num460 = Item.NewItem((int)projectile.Center.X, (int)projectile.Center.Y, 0, 0, ItemID.PlatinumCoin, (int)projectile.ai[1]);
			...
			if (projectile.alpha < 90 && Main.rand.Next(3) == 0)
			{
				...
				int num459 = Dust.NewDust(projectile.Center + value119, 0, 0, 247);
				Main.dust[num459].position = projectile.Center + value119;
				Main.dust[num459].velocity = Vector2.Zero;
			}
			float num458 = 96f / 255f;
			float num457 = 109f / 255f;
			float num456 = 144f / 255f;
